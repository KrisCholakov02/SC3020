    ...
    if (rootAddress != nullptr) {
        ...
        while (!temp->isLeafNode()) {
            ...
            for (int j = 0; j < temp->numKeys; j++) {
                ...
                if (j == temp->numKeys - 1) {
                    ...
                    temp = (TreeNode *) indexes->loadRecordFromStorage(temp->pointers[j + 1], nodeSize);
        ...
        while (!finish) {
            ...
            for (int k = 0; k < temp->numKeys; k++) {
                ...
                if (temp->keys[k] != end && temp->pointers[temp->numKeys].blockAddress != nullptr ) {
                    // Load the right sibling of temp into temp,
                    temp = (TreeNode *) indexes->loadRecordFromStorage(temp->pointers[temp->numKeys], nodeSize);
                    cout << "Node Accessed";
